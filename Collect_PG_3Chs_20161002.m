...
[XN, XC] = rnd_smp_PG_3Chs_kNN_20161002(TrainingNoisy, TrainingClean, num_patch_N, num_patch_C,  par);

for cc = size(XN,2)
    [modelcc, llh, cls_idx]  =  empggm(XC{cc}, cls_num,par.nlsp);
    model{cc,1} = modelcc;
    for c = 1 : cls_num 
        idx = find(cls_idx  == c);
        %     if (length(idx) >  10000) 
        ...
        %     end 
        Xn{Patch_Channel,c} = XN{cc}(:, idx);
        Xc{Patch_Channel,c} = XC{cc}(:, idx);
    end
    % % add smooth patches
    % Xn{cls_num+1} = XN0;
    % Xc{cls_num+1} = XC0;
    ...
    % clear XN XC XN0 XC0;
    % save model
    GMM_model = ['Data/GMM_PG_' num2str(Patch_Channel) '_' num2str(par.nlsp) '_' num2str(patch_size) 'x' num2str(patch_size) '_' num2str(cls_num) '_' datestr(now, 30) '.mat'];
